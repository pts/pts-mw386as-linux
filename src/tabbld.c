...
static int ct, opcode, opt, optDoc;
...
buildOp()
{
	...
	for (p = optf; ; p++) {
		...
		case 'p':
			opt |= PFX_0F;		break;
		case 'm':
			opt |= MODRM_BYTE;	break;
		case 'a':
			opt |= ADD_REG;		break;
		case 'L':
			opt |= LOCK_OP;		break;
		case 'l':
			opt |= AFTER_LOCK;	break;
		case 'R':
			opt |= REP_INSTR;	break;
		case 'r':
			opt |= AFTER_REP;	break;
...
/*			((optDoc & INDEF_JMP) ? "ja" : opc), lineno); */
	else
	...
	if (f->operands) {
		...
		for (i = 0; i < f->operands; i++) {
			if (i)
				fprintf(odp, ",");
			fprintf(odp, "%s", opertab[f->ap[i]].name);
		}
		fprintf(odp, "\\fR");
	}
	if (*comment) {
...
opBld()
{
	...
	optDoc = opt;
	opt &= ~(INDEF_JMP | USE_REG);

	if (opc[0] == '0')	/* Ambigous match */
		opt |= AMBIG_MATCH;

	if (op3[0]) {
		ct = 3;
		sprintf(fname, "S_%x_%s_%s_%s", opt, op1, op2, op3);
	}
	else if (op2[0]) {
		ct = 2;
		sprintf(fname, "S_%x_%s_%s", opt, op1, op2);
	}
	else if (op1[0]) {
		ct = 1;
		sprintf(fname, "S_%x_%s", opt, op1);
	}
	else {
		ct = 0;
		sprintf(fname, "S_%x", opt);
	}
...
reorgData()
{
	...
	qsort(optab, opct, sizeof(*optab), compr);

	for (last[0] = nameCt = i = 0; i < opct; i++) { /* scan opcodes */
		this = optab + i;
		this->lineno = -1;
		if (-1 == this->fun) { /* general name */
			for (j = 0; j < opct; j++) { /* scan for reference */
				that = optab + j;
				if (-2 != that->hash ||
				    this->gen != that->gen)
					continue;
		...
		if (strcmp(last, this->name)) {
			strcpy(last, this->name);
			this->len = k;
			this->pt = j;
			nameCt++;
		}
...
outData()
{
	...
	for (i = 0; i < funct;) {
		f = funtab + i++;
		fprintf(ohp, "#define %-20s %d\n", f->name, i - 1);
		fprintf(ofp, " /* %-20s */ { %10s, 0x%04x, %d",
			f->name,
			f->type,
			f->opt & 0xffff,
			f->operands);
	 ...
	 * lineno is now used to point to the last name group
	 * in a generic opcode.
	 */
	fprintf(ofp, "readonly short prefTab[] = {\n");
	for (lastp = i = 0; i < opct; i++) {
		this = optab + i;
		if (this->fun == -1) {	/* generic opcode */
			work[0] = '\0';
			if (this->opcode = lastp)
				fprintf(ofp, ",\n");
			fprintf(ofp, "\t/* %-10s %d */\n", this->name, lastp);
			for (j = 0; j < opct; j++) {
				that = optab + j;
			...
			if (work[0])	/* mark last name in generic */
				optab[k].lineno = l;
			fprintf(ofp, "\t-1");
			lastp++;
		}
	...
	for (i = 0; i < nameCt;) {
		...
		memcpy(work, allntab + this->pt, this->len);
		work[this->len] = '\0';
		fprintf(ofp, "\t{%4d, %4d, %2d, %4d }%c /* %-12s %d */\n",
